class Color {constructor(r, g, b) {this.R = Math.floor(r);
    this.G = Math.floor(g);
    this.B = Math.floor(b);
  }
  localeCompare(that) {return this.toString().localeCompare(that.toString());
  }
  toString() {let r = (this.R < 16 ? '0' : '') + this.R.toString(16);
...
function findTargetColors(colors) {if (colors.length < 16) {return colors.sort();
    ...
    for (let i = 0; i < colors.length; i += 1) {
      ...
      let right = colors[++i];
      if (right) {
        newColors.push(new Color((left.R + right.R) / 2,
          (left.G + right.G) / 2,
          (left.B + right.B) / 2
        ));
...
function getImageColors(imgObj) {let canvas = document.createElement('canvas');
...
function getImage(data, callback) {let img = document.createElement('img');
  img.onload = e => {typeof callback === 'function' && callback(img);
  };
  img.src = data;
}

function setupInputFile() {let file = document.querySelector('#file');
  let img = document.querySelector('#input');
  let placeholder = document.querySelector('#placeholder');
  let output = document.querySelector('#output');
  let loading = document.querySelector('#loading');
  file.onchange = e => {if (file.files.length) {let infile = file.files[0];
      if (infile.type.startsWith('image/')) {let reader = new FileReader();
        ...
        reader.onload = e => {
          ...
          getImage(reader.result, imgObj => {
            ...
            imgColors.forEach(color => {let div = document.createElement('div');
